...
function AuthContextProvider(props) {
  ...
  const fetchDataList = async (num) => {
    ...
    const page = res?.data?.filter((item,i)=> i+1 <=6 * num && i >=(num-1) * 6)
  ...
  return (
    <AuthContext.Provider value={{ postList, setPostList, fetchDataList, setCurrentUser, currentUser, fetchDataLogin, pageCount }}>
      {props.children}
